...
export const register = async (req, res) => {
    ...
    try {
        ...
        if (password.length >= 6 && regex.test(password)) {
            ...
            //creating jwt token
            const id = user._id
            jwt.sign({ id, name, email }, process.env.JWT_SECRET, {expiresIn: '1d'}, (err, token) => {
                if (err) {
                    res.status(500).json({ error: "Error in creating token" });
                }
                const created_at = user.createdAt;
                //generating response format as required
                const response = genResponseFromat(id, name, email, created_at, token)
                res.cookie('token', token, {HttpOnly: true, maxAge: 24 * 60 * 60 * 1000}).json(response)
            })
        }
        else {
...
export const login = async (req, res) => {
    ...
    try {
        ...
        if (user) {
            if (!user.password) {
                return res.status(400).json({ error: 'Try login in with google' });
            }
            ...
            if (passwordCompare) {
                ...
                jwt.sign(
                    { id: user._id, email: user.email, name: user.name, }, process.env.JWT_SECRET,
                    { expiresIn: '1d' }, (err, token) => {
                        ...
                        //generating response format as required
                        const response = genResponseFromat(user.id, user.name, user.email, created_at, token)
        ...
        const response = {
            ...
            content: {
                data: {
                    name: user.name, 
                    email: user.email,
                    picture: user.picture || "https://gravatar.com/avatar/5c9e856d9dfff899009faf44b9bee015?s=400&d=robohash&r=x",
